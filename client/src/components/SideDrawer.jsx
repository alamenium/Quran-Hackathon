...
import { characterImage, profileImage } from '../lib/characters.js';
import { api } from '../lib/api.js';

// Side drawer — slides in from the right when the menu button in the
// Header is tapped.
//
// Quran Foundation User API (OAuth2):
//   Shows login state and a "Connect Quran Foundation account" button.
//   When logged in, the QF user's name is shown and bookmarks/notes/streaks
//   are stored in the QF User API instead of the local store.
//   Tokens are server-side only (httpOnly cookie). The React app only sees
//   the session status from /api/auth/me — never the tokens themselves.
export function SideDrawer({ open, onClose }) {
  const { user } = useProgress();
  const navigate = useNavigate();
  // QF OAuth2 session state: null=loading, { authenticated: false }, or { authenticated: true, user }
  const [qfAuth, setQfAuth] = useState(null);

  // Fetch QF login state whenever the drawer opens.
  useEffect(() => {
    if (!open) return;
    api.auth.me()
      .then(setQfAuth)
      .catch(() => setQfAuth({ authenticated: false }));
  }, [open]);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Quran Foundation account — OAuth2 login/logout */}
        <div className="px-4 py-3 border-b border-gray-100">
          {qfAuth === null && (
            <div className="text-xs text-ink-faint">Checking account…</div>
          )}
          {qfAuth?.authenticated ? (
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-brand-100 flex items-center justify-center text-brand-600 font-extrabold text-sm shrink-0">
                {(qfAuth.user?.firstName?.[0] || '?').toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-xs font-extrabold text-brand-600 leading-tight truncate">
                  {qfAuth.user?.firstName} {qfAuth.user?.lastName}
                </div>
                <div className="text-[10px] text-ink-faint">
                  Quran Foundation · bookmarks &amp; notes synced
                </div>
              </div>
              <button
                type="button"
                onClick={api.auth.logout}
                className="text-[10px] font-bold text-ink-soft hover:text-accent-pink transition px-2 py-1 rounded-lg hover:bg-pink-50"
              >
                Log out
              </button>
            </div>
          ) : qfAuth?.authenticated === false ? (
            <button
              type="button"
              onClick={api.auth.login}
              className="w-full flex items-center gap-2 bg-brand-50 border-2 border-brand-200 rounded-2xl px-3 py-2.5 hover:bg-brand-100 transition text-left"
            >
              <span className="text-lg" aria-hidden>🔑</span>
              <div className="flex-1 min-w-0">
                <div className="text-xs font-extrabold text-brand-600 leading-tight">
                  Connect Quran Foundation
                </div>
                <div className="text-[10px] text-ink-faint">
                  Sync bookmarks, notes &amp; streak
                </div>
              </div>
            </button>
          ) : null}
